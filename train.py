...
def get_args_parser():
    ...
    parser.add_argument('--gpt_loss_coefficient', default = 0.1, type = float,
...
def main(args):
    ...
    pwd = args.root_path
    data_root = os.path.join(pwd, args.dataset_folder)
    ...
    with tqdm(total = len(dataset) // batch_size) as t:
        for step in range(len(dataset) // batch_size):
            ...
            for update_time in range(update_time_per_episode):
                ...
                t.set_postfix(klloss = float(kl_losses[-1]), float(gpt_loss = gpt_losses[-1]),
